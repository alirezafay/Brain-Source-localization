...
Threshold_MNE = 0:0.01:201;
AUC_MNE = 0;
for i=1:length(Threshold_MNE)
    val_MNE = zeros(1,p);
    for j=1:p
        if (amplitude_MNE(j)>Threshold_MNE(i))
            val_MNE(j) = 1;
        end
    end
    Confusion_matrix = confusionmat(val_real,val_MNE);
    ...
    if i~=1
        AUC_MNE =TPR(i)*(FPR(i-1)-FPR(i)) +AUC_MNE ;
    end
end
plot(FPR,TPR)
title('ROC Of MNE Algorithm')
xlabel('1-Specificity')
ylabel('Sensitivity')
[s ,Optimum_Index] = min(asses);
Threshold_Optimum_MNE = Threshold_MNE(Optimum_Index);
Accuracy_Optimum_MNE = Accuracy(Optimum_Index);
precision_optimum_MNE = precision(Optimum_Index);
FPR_MNE = FPR(Optimum_Index);
